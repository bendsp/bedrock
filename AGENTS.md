...
- 2026-03-22: Tightened hybrid table rendering so malformed contiguous table blocks fully fall back to raw Markdown, added idle inline-Markdown cell rendering with raw-on-focus editing, and routed formatting shortcuts into the active table cell editor.
- 2026-03-22: Isolated focused table cell editors from the outer CodeMirror event stream, reused inline hybrid markdown styling while editing, and normalized a guaranteed blank line plus click target after tables so post-table typing cannot corrupt the last row.
- 2026-03-23: Added a persisted font-family setting with curated Bedrock/System/Inter/Satoshi/Serif options, live CSS variable plumbing for app and editor prose fonts, and fallback-safe handling that leaves explicit monospace surfaces unchanged.
